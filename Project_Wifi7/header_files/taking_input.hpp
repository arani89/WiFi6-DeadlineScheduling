    ...
    for(int st = 0; st < stations; st++){
        ...
        while(sumArrivalTimes < T)
        {
            ...
            for(int i = 0; i < pcktnodes[st]; i++){
                Packet pckt(pcktpenalty[st], sumArrivalTimes+start_time, pcktsize[st], sumArrivalTimes+start_time+pcktdelay[st], n_stations+i, universalCounter++, st, ((double)random(0, 1000)/1000));
    ...
    for(auto &pro: producers){
        ...
        for(double arri = start_time; arri < (double)T+start_time; arri += Tp){
            for(int ctr = 0; ctr < nodes; ++ctr){
                Packet pckt(penalty, arri, size, arri + delay, n_stations+ctr, universalCounter++, producer_i, 1);
                packets.push_back(pckt);
                if(pckt.penalty >= critical_threshold){
                    totalcritical++;
                }
...
int usecase1Input(int T, vector<Packet>& packets, int start_time, vector<vector<double>> &producers){
    ...
    for(auto &pro: producers){
        ...
        for(double arri = start_time; arri < (double)T+start_time; arri += Tp){
            for(int ctr = 0; ctr < nodes; ++ctr){
                Packet pckt(penalty, arri, random(minsize, maxsize), arri + delay, n_stations+ctr, universalCounter++, producer_i, 1);
                if(pckt.penalty >= critical_threshold){
                    totalcritical++;
                }
                packets.push_back(pckt);
...
int usecase2Input(int T, vector<Packet>& packets, int start_time, vector<vector<double>> &producers){
    ...
    for(auto &pro: producers){
        ...
        for(double arri = start_time; arri < (double)T+start_time; arri += Tp){
            for(int ctr = 0; ctr < nodes; ++ctr){
                Packet pckt(penalty, arri, 10, arri + delay, n_stations+ctr, universalCounter++, producer_i, 1);
                if(pckt.penalty >= critical_threshold){
                    totalcritical++;
                }
                packets.push_back(pckt);
        ...
        if(offered_load > 0){
            ...
            while(sumArrivalTimes < timeperiod){
                ...
                for(int i = 0; i < nodes; i++){
                    ...
                    Packet pckt(ndpProfit, sumArrivalTimes+start_time, datasize, sumArrivalTimes+ndpDelay+start_time, n_stations+i, universalCounter++, st, 1);
                    total_size+=datasize;
                    if(pckt.penalty >= critical_threshold){
                        totalcritical++;
                    }
