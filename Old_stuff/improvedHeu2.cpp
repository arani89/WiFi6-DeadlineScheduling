...
//------------------- Interval Generation -------------------
vector<double> printer1(0), printer2(0);
//global variables:
...
int createInterval(vector<Packet>& packets, int T){
    ...
    printer1.push_back((double)(totalDropped-criticalDropped)/(double)lPackets);
    printer2.push_back((double)(criticalDropped)/(double)lPackets);
    return score;
}
// ----------------------------------------------------------
// -----------------------BASELINES-------------------------
...
int baselineNLRF(vector<Packet> packets, int T){
    ...
    printer1.push_back((double)(totalDropped-criticalDropped)/(double)lenPackets);
    printer2.push_back((double)(criticalDropped)/(double)lenPackets);
...
int main(){
    ...
    while(T--){
        ...
        printer1.clear(); printer2.clear();
		for(int i = 0; i < ditr; i++)
            createInterval(packets[i], timeperiod);
			// cout<<((double) createInterval(packets[i], timeperiod)/(double) totalProfits[i])<<" ";
		for(auto hig: printer1) cout<<hig<<" ";
        cout<<"\n";
        for(auto hig: printer2) cout<<hig<<" ";
        cout<<"\n";
        printer1.clear(); printer2.clear();
		for(int i = 0; i < ditr; i++)
            newBaseline(packets[i], timeperiod, EDF);
        ...
        printer1.clear(); printer2.clear();
		for(int i = 0; i < ditr; i++)
            newBaseline(packets[i], timeperiod, LRF);
        ...
        printer1.clear(); printer2.clear();
		for(int i = 0; i < ditr; i++)
            baselineNLRF(packets[i], timeperiod);
