...
z = p_model.binary_var_matrix(T, T, name = 'z')
x = p_model.binary_var_cube(k, T, T, name = 'x')
lam = p_model.binary_var_list(k, name = 'lam')
...
p_model.add_constraints( sum( 
    (sum(x[pkt,i,j] for j in range(i, T))) for i in range(arri[pkt],ded[pkt] - tdur[pkt] + 1)
...
p_model.add_constraints( x[pkt,i,j] == 0 for pkt in range(k) 
                        for i in range(T) for j in range( min(i+tdur[pkt], T)));
