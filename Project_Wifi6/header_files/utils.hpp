...
double calcTransmissionTimeMs(double size, int mode, int bitmode, int mcs_range, int mcs_lowerval, int rtLoss)
{
    double bitrate;
    ...
    if (mode < RU_1992)
        ...
        bitrate = -1;
    double rtlf = (rtLoss==1 ? 0.3 : 0.0);
    int retransmission_factor = 1;
    // if(rtLoss==1)
    //     retransmission_factor = random(1, 7);
    // cout<<rtlf<<endl;
    // 
    double res = (size * 8.0) / ((bitrate * 1000.0)*(1.0 - rtlf));
    // double res = (size * 8.0 * retransmission_factor) / (bitrate * 1000.0);
    return res;
}
