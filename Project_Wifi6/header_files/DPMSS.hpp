...
config, which
*********************************/
vector<vector<int>> selectIndicesForConfig(int itvl_start, int itvl_end, vector<int> &availablePackets, vector<int> &newAvailablePackets, vector<Packet> &packets, double granularity, double packetDropFactor, Config &userconfig, int rtLoss)
{
...
vector<IntervalData> LSDS(vector<Packet> &packets, int start_time, int T, int stations_count, int criticalThreshold, double granularity, double packetDropFactor, Config &userconfig)
{
    ...
    for (int d = 1; d <= delta; ++d)
    {
        for (int t = start_time; t < T - d + start_time; ++t)
        {
            ...
            vector<vector<int>> bestids = selectIndicesForConfig(intervalStart, intervalEnd, availableIndices, newAvailableIndices, packets, granularity, packetDropFactor, userconfig, 0);
            // calc prefix sum
